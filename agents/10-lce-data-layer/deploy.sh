...
# Deploy to AWS AgentCore — run from inside agent-10 folder
# ================================================================
set -e

AGENT_NAME="lceDataLayer"
S3_KEY="lce-data-layer-v1.zip"
...
CREATE_RESP=$(aws bedrock-agentcore-control create-agent-runtime \
  ...
  --description "LCE Data Layer — Phase 1 Agent 10 — No RDS, GHL + live APIs" \
...
if [ -z "$RUNTIME_ID" ]; then
  echo "ERROR: Copy runtime ID manually"
  exit 1
fi
